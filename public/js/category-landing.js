...
(function() {
    ...
    fetch(`http://127.0.0.1:8001/partials/categories/${categorySlug}/data`)
        .then(response => {
            if (!response.ok) {
    ...
    function updateHTMXEndpoints(slug) {
        ...
        if (tourResults) {
            const newEndpoint = `http://127.0.0.1:8001/partials/tours/search?category=${slug}&per_page=12`;
    ...
    function updateRelatedCategoriesEndpoint(slug) {
        ...
        if (relatedSection) {
            const newEndpoint = `http://127.0.0.1:8001/partials/categories/related?current=${slug}&limit=5`;
    ...
    if (resetButton) {
        resetButton.addEventListener('click', function(e) {
            ...
            htmx.ajax('GET', `http://127.0.0.1:8001/partials/tours/search?category=${categorySlug}&per_page=12`, {
                target: '#tour-results',
                swap: 'innerHTML'
            });
        });
